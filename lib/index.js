...
async function runTest() {
  if (scenarios.length > 0) {
    ...
    const logDir = `${root}/${logDirname}/${filename}`;
    ...
    if (!fs.existsSync(`${root}/${logDirname}`)) {
      fs.mkdirSync(`${root}/${logDirname}`);
    }
    ...
    (async function processStep(stepIdx = 0) {
      if (stepIdx < steps.length) {
        ...
        if (!fs.existsSync(`${logDir}/${stepIdx}`)) {
    ...
    if (results.every(([file, result]) => result)) {
      process.exit(0);
    } else {
      process.exit(1);
    }
...
const logDirname = argv.logDir || "logs";
